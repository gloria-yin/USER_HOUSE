...
3. 每移动一次，棋盘会新增一个数字块。
4. 爽玩版更容易铺开，最终分数减半。
5. 棋盘被塞满且无法移动时结束，数字可继续合成到 65536。
...
1. 每局先选择难度：简单 144 张，困难 210 张且牌面更密。
2. 点击没有被上层遮挡的牌放入下方槽位。
3. 同图标凑满 3 张会消除。
4. 槽位超过 7 格且没有消除时失败。
5. 难度会影响最终分数倍率。
...
1. 每局自动生成至少 20 块分层面板，题面不同，并按颜色三倍数生成螺丝。
2. 顶部同时显示 3 个随机颜色工具盒；同色螺丝进入对应盒子，盒子收满 3 颗会自动打包并刷新下一个颜色。
3. 不匹配当前工具盒的螺丝进入 5 格临时托盘；托盘满 5 格后，再点击一颗无法直接进入工具盒的螺丝时失败。
4. 上层面板会遮挡下层螺丝，只有露出的螺丝可以点击。
5. 面板剩 1 颗螺丝时会以该螺丝为轴摆动；失去全部螺丝后受重力下落。
6. 工具盒队列覆盖所有螺丝，清空全部面板后结算。

### 双人游戏

双人游戏会让你和 char 轮流行动。语录更偏互动，会根据胜负、堵路、抽牌、危险局面等事件变化。

支持耍赖模式的双人游戏，每局最多可以点击 3 次「耍赖」撤销上一步操作。耍赖会触发专属语录，并在游戏日志里记录本局反悔次数。
...
4. 被跳过的棋子可以是自己或对方；连续跳跃时会一步一步动画展示。
5. 先让 10 颗棋子全部进入对方营地的一方获胜。
